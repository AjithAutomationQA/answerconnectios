...
public class BasePages extends BaseTest {
	...
	public Boolean checkIsFutureDate(String date) {
		...
		if (date.contains("Jan") || date.contains("Feb") || date.contains("Mar") || date.contains("Apr")
				...
				|| date.contains("Sep") || date.contains("Oct") || date.contains("Sep") || date.contains("Oct")
	...
	public void datePicker(String hours, String minutes) throws InterruptedException {
		...
		// driver.findElementsByXPath("//XCUIElementTypePickerWheel");
	...
	// -----------------Created by Ajith Kumar 114171 ------------------- //

	public String getText(WebElement locator) {
		wait(locator);
		String elementName = locator.getText();
		System.out.println(elementName);
		childTest.log(Status.PASS, "Text retrieved: " + elementName);
		return elementName;
	...
	public void isDisplayed(WebElement locator) {
		...
		childTest.log(Status.PASS, "Element is displayed in the current page");
	}

	public void tapTheElement(WebElement element, String name) throws InterruptedException {

		isDisplayed(element);
		element.click();
		childTest.log(Status.INFO, "Tapped on ->" + name);
		System.out.println("Tapped on ->" + name);
	}

	public void sendKey(WebElement element, String input, String name) {

		isDisplayed(element);
		element.sendKeys(input);
		childTest.log(Status.INFO, "Entered[ " + input + "]in " + name);
		System.out.println("Entered[ " + input + "]in " + name);

	}

	public void scrollDown(WebElement element, String name) {
	...
	public void xpathByText(String text, String name) {
		// XCUIElementTypeStaticText[@name=""]

		String locator = "//XCUIElementTypeStaticText[@name=\"" + text + "\"]";
	...
	public void scrollDown1(String name) throws Throwable {
		// public void scrollDown1(List<WebElement> e1, String input) throws Throwable {

		// new WebDriverWait(driver,
		// 10).until(ExpectedConditions.presenceOfElementLocated(By.xpath("//*[@XCElementType='XCUIElementTypeStaticText']")));
		// driver.findElement(By.xpath("//*[@id='inboundmessagecell_title']")).click();
//		try {
		List<IOSElement> d = driver
				.findElements(By.xpath("(//XCUIElementTypeStaticText[@name=\"inboundmessagecell_title\"])"));
//		
		int rowcountofthelist = d.size();
		System.out.println("No of elements : " + rowcountofthelist);
//		
//
		for (int DropdownValues = 1; DropdownValues <= rowcountofthelist; DropdownValues++) {

			WebElement Dfow = driver.findElement(By.xpath(
					"(//XCUIElementTypeStaticText[@name=\"inboundmessagecell_title\"])[" + DropdownValues + "]"));
			String DfowInDropdown = Dfow.getText();
			System.out.println(DfowInDropdown);

			if (name.equalsIgnoreCase(DfowInDropdown)) {
				System.out.println(
						"(//XCUIElementTypeStaticText[@name=\"inboundmessagecell_title\"])[" + DropdownValues + "]");
				System.out.println("matched");
				HashMap<String, Object> scrollObject = new HashMap<String, Object>();
//				 scrollObject.put("element", ((RemoteWebElement) Dfow).getId());
				scrollObject.put("direction", "down");
				scrollObject.put("xpath", Dfow);
				driver.executeScript("mobile: scroll", scrollObject);
				...
				// tapOn("message", Dfow);
				// Dfow.click();
				HashMap<String, Object> scrollObject2 = new HashMap<String, Object>();
				scrollObject2.put("xpath", Dfow);
				driver.executeScript("mobile: tap", scrollObject2);
...
//				List<WebElement> d = e1;
//				for (WebElement object : e1) {
//					String Message = object.getText();
//					if (Message.equalsIgnoreCase(input)) {
//						HashMap<String, Object> scrollObject = new HashMap<String, Object>();
//						scrollObject.put("direction", "down");
//						driver.executeScript("mobile: scroll", scrollObject);
